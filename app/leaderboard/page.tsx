...
function RankBadge({ rank }: { rank: number }) {
  ...
  const defaultStyle = "bg-cyan-500/10 text-cyan-400 ring-cyan-500/20";
...
function FollowButton() {
  return (
    <button
      type="button"
      className="rounded-full bg-cyan-500/10 px-3 py-1 text-xs font-medium text-cyan-400 ring-1 ring-cyan-500/20 transition hover:bg-cyan-500/20"
    >
      Follow
    </button>
  );
...
export default function LeaderboardPage() {
  ...
  async function fetchTopTraders() {
    try {
      ...
      setError("Unable to load trader rankings. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="mb-8 flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl shadow-black/20 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-medium text-cyan-400">Leaderboard</p>
            ...
            <p className="mt-2 text-sm text-slate-400">
              ...
              and discover successful trading strategies across multiple
              prediction markets.
            </p>
          </div>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              type="button"
              onClick={() => void fetchTopTraders()}
              disabled={isLoading}
              className="inline-flex items-center gap-2 rounded-xl border border-cyan-500/20 bg-cyan-500/10 px-4 py-2 text-sm font-medium text-cyan-400 transition hover:bg-cyan-500/20 disabled:cursor-not-allowed disabled:opacity-60"
            >
          ...
          <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <select
              ...
              className="rounded-xl border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-white focus:border-cyan-500 focus:outline-none"
            >
